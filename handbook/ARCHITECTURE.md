...
- Main-session runtime detection is tool-activity driven (assistant tool calls + tool results) with session lock awareness for in-flight turns, so plain chat back-and-forth does not trigger `RUNNING` by itself.
